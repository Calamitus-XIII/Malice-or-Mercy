...
public class FormidableEnchantmentProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, Entity entity, Entity sourceentity) {
		...
		if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).isEnchanted()) {
			if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getEnchantmentLevel(MaliceOrMercyModEnchantments.FORMIDABLE.get()) == 1) {
				if (Math.random() < 0.25) {
						...
						_entity.addEffect(new MobEffectInstance(MaliceOrMercyModMobEffects.FEAR.get(), 1200, 0));
					{
						...
						if (!_ent.level().isClientSide() && _ent.getServer() != null) {
							_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4,
									_ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), "execute at @s run playsound minecraft:block.anvil.land master @a 10 0");
						}
					}
				}
			}
